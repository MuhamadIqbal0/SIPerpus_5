...
buku"
    ...
    <div class="field">
      ...
      <div class="radio-group">
        <label><input type="radio" v-model="form.kondisi" value="baru" /> Baru</label>
        <label><input type="radio" v-model="form.kondisi" value="baik" /> Baik</label>
        <label><input type="radio" v-model="form.kondisi" value="rusak" /> Rusak</label>
      </div>
    </div>
    ...
    <div class="preview" v-if="form.judul">
      <h4>Preview:</h4>
      <p>
        <strong>{{ form.judul }}</strong> oleh {{ form.penulis }}
      </p>
      <p>Kategori: {{ form.kategori }} | Kondisi: {{ form.kondisi }}</p>
      <p>Tag: {{ form.tags.join(', ') || 'Belum dipilih' }}</p>
    </div>
    <button type="submit" :disabled="!isFormValid" class="btn-submit">Simpan Buku</button>
  </form>
</template>
<script setup>
...
const daftarTag = ['Rekomendasi', 'Buku Baru', 'Populer', 'Langka', 'Referensi']

// validasi sederhana
const isFormValid = computed(() => {
  return form.judul.trim().length >= 3 && form.penulis.trim().length >= 3 && form.kategori !== ''
})
